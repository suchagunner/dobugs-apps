...
const getScriptUrl = (clientId) => `https://dapi.kakao.com/v2/maps/sdk.js?appkey=${clientId}`;
...
export default class KakaoMap {
  ...
  initMap(el: HTMLElement, options = {}) {
    ...
    this.map = new window.kakao.maps.Map(el, {
      center: new window.kakao.maps.LatLng(37.5729503, 126.97935789999997),
      ...options,
    });
    this.initiated = true;
  ...
  addMapEventListener(eventName: string, callback: Function) {
    const listener = window.kakao.maps.Event.addListener(this.map, eventName, callback);
  ...
  triggerEvent(target, eventName, eventObject) {
    window.kakao.maps.Event.trigger(target, eventName, eventObject);
  }
  ...
  createMarker(
    ...
    const position = new window.kakao.maps.LatLng(options.latitude, options.longitude);
    ...
    const marker = new window.kakao.maps.Marker({
      ...
      icon: {
        ...
        size: new window.kakao.maps.Size(42, 50),
        scaledSize: new window.kakao.maps.Size(42, 50),
        origin: new window.kakao.maps.Point(0, 0),
        anchor: new window.kakao.maps.Point(21, 0),
      },
    });
  ...
  addMarkerEventListener(marker: any, eventName: string, callback: Function) {
    ...
    const listener = window.kakao.maps.Event.addListener(marker, eventName, (e) => {
      callback(e);
    });
  ...
  getDistanceBetween(pointA, pointB) {
    const points = [pointA, pointB].map(
      (point) => new window.kakao.maps.LatLng(point?.latitude || point?.x, point?.longitude || point?.y)
    );
